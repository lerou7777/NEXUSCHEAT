...
export function AnalysisModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!accountId.trim()) {
      setError('Informe o ID da conta para continuar.');
      return;
    }

    if (!/^\d+$/.test(accountId)) {
      setError('O ID deve conter apenas números.');
      return;
    }
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">
              Remove Roblox restriction v2
            </h2>
            <p className="text-sm text-muted-foreground">
              {gameName} • Verificação de conta
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              O ID será usado apenas para realizar a análise automatizada da conta.
            </p>
